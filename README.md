<h1 align="center">Hi there 👋, I'm Nesma Hesham</h1>
<h3 align="center">🎓 3rd-year Computer Science Student | 💻 Passionate about Tech & Learning</h3>

---


## 👩‍💻 About Me

🎓 3rd‑year student at **Benha Faculty of Computers and Artificial Intelligence (2023–2027)**  
💡 Passionate about **Software Engineering**, **Front-End Development**, and continuously expanding my skill set  
📊 Interested in **Web Development**, **Data Analysis & Visualization**  
🌱 Currently improving skills in `C++`, `Python`, `Java`, `OOP`, `Problem Solving`, `Algorithms`, `SQL`, `HTML5`, `CSS3`, and `MIPS Assembly`  
🧠 Competitive Programmer with strong problem-solving skills, creative mindset & solid foundation in algorithms  
🤝 **URI Member** – *Public Relations Committee*: promoting events and strengthening our tech community  
💼 Excellent in communication, teamwork, and time‑management  

---

## 🏆 Experience & Involvement

### 🧑‍🏫 Problem-Solving Mentor – ACBC Club, Banha University
- Mentoring junior students in competitive programming and algorithmic thinking  
- Preparing training materials and guiding problem-solving sessions  
- Supporting team participation in local and national coding contests  

### 🎯 ECPC – Egyptian Collegiate Programming Contest (Alexandria)
- Competed in high‑pressure coding challenges under strict time limits  
- Enhanced algorithmic thinking and collaborative coding strategies  

### 📊 Data Science Bootcamp – GDG (2025)
- Built and evaluated ML models using `Python`, `Pandas`, and `NumPy`  
- Created interactive dashboards and visualizations with `Power BI`  

### 💻 Frontend Bootcamp – GDG (2024)
- Developed responsive web interfaces with `HTML`, `CSS`, and modern UI principles  
- Took part in a final hackathon, delivering a polished, user‑friendly prototype  

### 🧠 Career Explorer Bootcamp – Soft Skills (2024)
- Strengthened communication, leadership, and time‑management abilities  
- Learned best practices for professional growth and collaboration  

---

## 🛠️ Skills

### Programming Languages:
**C++** · **Python** · **Java** · **HTML5** · **CSS3** · **SQL** · **MIPS Assembly**

### Technologies & Tools:
**Git** · **GitHub** · **VS Code** · **Power BI** · **Pandas** · **NumPy**

### Competitive Programming:
**Codeforces** · **ICPC** · **ECPC**

---


## 🌐 Languages

- 🇪🇬 **Arabic** – Native  
- 🇬🇧 **English** – Very Good  
- 🇫🇷 **French** – Elementary

---

## 📊 GitHub Stats

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=Nesma-h&hide_border=true&show_icons=true)

![GitHub Streak](https://streak-stats.demolab.com/?user=Nesma-h&theme=radical)

![GitHub Followers](https://img.shields.io/github/followers/Nesma-h?style=social)

![Profile Views](https://komarev.com/ghpvc/?username=Nesma-h&color=blue)

![Most Used Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=Nesma-h&layout=compact&theme=radical)



## 📬 Let's Connect

[![GitHub](https://img.shields.io/badge/GitHub-Nesma--h-181717?style=flat&logo=github)](https://github.com/Nesma-h)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Nesma_Hesham-0077B5?style=flat&logo=linkedin)](https://www.linkedin.com/in/nesmaa-heesham)
[![Codeforces](https://img.shields.io/badge/Codeforces-N__esma-1F8ACB?style=flat&logo=codeforces)](https://codeforces.com/profile/N_esma)
[![Email](https://img.shields.io/badge/Email-nesma@ahh1@gmail.com-D14836?style=flat&logo=gmail)](mailto:nesma@ahh1@gmail.com)

---

<p align="center">✨ Always eager to learn, collaborate, and solve real-world problems through tech 🚀</p>
